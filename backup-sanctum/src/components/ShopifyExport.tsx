...
export default function ShopifyExport({ isInline }: ShopifyExportProps) {
  ...
  // Auto-detect and fetch live products from the secure server-side Shopify endpoint
  useEffect(() => {
    const autoLoadShopify = async () => {
      try {
        setIsLoading(true);
        // First check if credentials are configured on the server
        const configRes = await fetch("/api/shopify-config");
        if (configRes.ok) {
          const config = await configRes.json();
          if (config.configured && config.domain) {
            setShopUrl(config.domain);
            setIsConnected(true);
            await fetchShopifyProducts();
          } else {
            // No server-side credentials — silently fall back to curated defaults
  ...
  /**
   * Fetch products from the secure server-side Shopify endpoint.
   * Credentials never leave the server — the browser only calls our own API.
   */
  const fetchShopifyProducts = async () => {
    ...
    try {
      const response = await fetch("/api/shopify/products?limit=6");

      if (response.status === 503) {
        // Credentials not configured on the server — fall back gracefully
        setIsConnected(false);
        setProducts(CURATED_DEFAULT_PRODUCTS);
        setIsLoading(false);
        return;
      ...
      const incoming: Product[] = resJson.products ?? [];
      ...
      setIsConnected(true);
      if (resJson.store) setShopUrl(resJson.store);
      setSuccessMessage(`LIVE SECURE SERVER-SIDE HANDSHAKE: ${(resJson.store || shopUrl).toUpperCase()}`);

      window.dispatchEvent(new CustomEvent("telemetry-log", {
        detail: {
          message: `SHOPIFY_SYNC: ${incoming.length} products fetched via secure server-side Storefront API proxy.`,
          type: "FORGE_SYNC"
        }
      }));
    } catch (err: any) {
      console.error("Shopify products fetch error:", err);
      setErrorMessage(`INTEGRATION REJECTED: ${err.message || "Check server logs for details."}`);
      setIsConnected(false);
      setProducts(CURATED_DEFAULT_PRODUCTS);
    } finally {
      setIsLoading(false);
  ...
  const disconnectStore = () => {
    ...
    window.dispatchEvent(new CustomEvent("telemetry-log", {
      detail: { message: "SHOPIFY_NODE: Disconnected — reverting to curated standalone catalog.", type: "WARNING" }
    }));
  };

  const handleTestConnect = () => {
    // Credentials are managed server-side via environment variables.
    // This triggers a fresh fetch from the secure server-side proxy.
